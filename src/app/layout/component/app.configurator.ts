...
export class AppConfigurator {
    ...
    ngOnInit() {
        if (isPlatformBrowser(this.platformId)) {
            // Ensure black-by-default and a dark surface on first load
            const cfg = this.layoutService.layoutConfig();

            if (!cfg.primary || cfg.primary === 'emerald' || cfg.primary === 'green') {
                this.layoutService.layoutConfig.update((s) => ({ ...s, primary: 'noir' }));
            }
            if (!cfg.surface) {
                // good dark neutral base
                this.layoutService.layoutConfig.update((s) => ({ ...s, surface: 'zinc' }));
            }
    ...
    getPresetExt() {
        ...
        if (color.name === 'noir') {
            // Map primary scale to surface scale (keeps your chips/tints consistent)...
            const primaryScaleFromSurface = {
                50: '{surface.50}',
                100: '{surface.100}',
                200: '{surface.200}',
                300: '{surface.300}',
            ...
            return {
                semantic: {
                    primary: primaryScaleFromSurface,
                    // Make the actual clickable/brand color BLACK in both schemes
                    colorScheme: {
                        light: {
                            primary: {
                                color: '{primary.950}', // nearly #000
                    ...
                    // Optional: make focus ring black instead of green
                    focusRing: { color: '#000000' }
                }
            };
        } else {
            if (preset === 'Nora') {
