...
export class CustomerBookingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Check if we should auto-open the booking form from dashboard
    this.checkForAutoOpenBookingForm();
  }

  private checkForAutoOpenBookingForm(): void {
    this.route.queryParams.subscribe(params => {
      ...
      // Check if editing a specific booking
      if (params['edit']) {
        const bookingId = parseInt(params['edit']);
  ...
  private loadBookingForEdit(bookingId: number): void {
    this.bookingService.getBookingById(bookingId).subscribe({
      next: (response: ApiResponse<Booking>) => {
        ...
        // Pre-fill form with existing booking data
        this.locationInput = booking.location;
        this.selectedServiceType = booking.service.toString();
        this.selectedDate = booking.booking_date;
        this.selectedTime = booking.time_slot;
        this.specialRequests = booking.special_requirements;
        
        // Pre-select cleaner if available
        if (booking.assign_cleaner) {
          // We need to wait for cleaners to load
          setTimeout(() => {
            const cleaner = this.availableCleaners.find(c => c.id === booking.assign_cleaner);
            if (cleaner) {
              this.selectedCleaner = cleaner;
            }
          }, 500);
