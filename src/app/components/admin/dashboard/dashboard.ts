...
import { ToastModule } from 'primeng/toast';
import { DashboardService, DashboardOverview, QuickStats, RecentBooking, RecentPayout } from '../../../services/dashboard/dashboard-service';
import { AuthService, User } from '../../../services/auth/auth-service'; // Import AuthService and User interface
...
export class Dashboard implements OnInit {
  ...
  loadCurrentUser(): void {
    ...
    if (this.currentUser) {
      // Use first name + last name, or username, or email as fallback
      if (this.currentUser.first_name && this.currentUser.last_name) {
        this.userName = `${this.currentUser.first_name} ${this.currentUser.last_name}`;
      } else if (this.currentUser.username) {
        this.userName = this.currentUser.username;
      } else if (this.currentUser.email) {
        this.userName = this.currentUser.email.split('@')[0]; // Use part before @ in email
      }
    }
  }
